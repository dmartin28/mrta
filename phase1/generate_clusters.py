        ...
        for i in range(len(clusters)):
            for j in range (i+1, len(clusters)):
                ...
                if len(clusters[i][0]) + len(clusters[j][0]) < L_r and len(clusters[i][1]) + len(clusters[j][1]) < L_t:
                    ...
                    merged_value = utils.coalition_value_2([robot_list[r] for r in merged_cluster[0]], [task_list[t] for t in merged_cluster[1]], kappa)

                    #Can probably store these values so we don't recompute each time:
                    clusteri_val = utils.coalition_value_2([robot_list[r] for r in clusters[i][0]], [task_list[t] for t in clusters[i][1]], kappa)
                    clusterj_val = utils.coalition_value_2([robot_list[r] for r in clusters[j][0]], [task_list[t] for t in clusters[j][1]], kappa)
