...
def coalition_value(robots, tasks):
    ...
    kappa = len(robots[0].capabilities)
    m = len(tasks)
    n = len(robots)
    ...
    for task in tasks:
        task_shapleys = shapley_vector(task)
        for i in range(kappa):
            shapley_vectors[i] = np.concatenate([shapley_vectors[i],task_shapleys[i]])
    
    # Sort shapley vectors
    for i in range(kappa):
        shapley_vectors[i] = np.sort(shapley_vectors[i])
    
    coalition_capabilities = np.zeros(kappa)
    for robot in robots:
        coalition_capabilities += robot.get_capabilities()
    
    # Calculate the total reward from capability matching
    capability_reward = 0
    for capability in range(kappa):
        for j in range(coalition_capabilities[capability]):
