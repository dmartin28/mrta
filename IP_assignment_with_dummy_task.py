...
def IP_assignment_with_dummy_task(robot_list, task_list, L):
    ...
    partitions = utils.generate_partitions(n, m+1, L)

    print(f"All partitions of {n} into {m} +1 parts with each part <= {L}:")
    ...
    for partition_idx in range(0,len(partitions)):
        for task_idx in range(0, m):
            # Have to add 1 to task_idx because partitions start with dummy task T0
