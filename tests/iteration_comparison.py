...
for size, avg_rewards in avg_results.items():
    plt.plot(range(1, num_iterations + 1), avg_rewards, label=f'Cluster Size {size}')

plt.xlabel('Iteration')
plt.ylabel('Average Iteration Reward')
plt.title('Average Iteration Reward vs. Iteration for Different Cluster Sizes')
plt.legend()
plt.grid(True)
plt.savefig('random_cluster_results_plot.png')
plt.show()

print("Plot saved as 'random_cluster_results_plot.png'")